...
static bool	Intern_makeForm(void);
...
TEST_START(Intern_makeForm)
	AForm	*shrubber = NULL;
	AForm	*robotomy = NULL;
	AForm	*pardon = NULL;
	TEST_LOGIC_START
		const Bureaucrat	mighty("Mighty", 1);
		const Intern		intern;

		shrubber = intern.makeForm("shrubbery creation", "gallery");
		robotomy = intern.makeForm("robotomy request", "Bobby");
		pardon = intern.makeForm("presidential pardon", "Toriel");
		intern.makeForm("saweerwdd fsefesf", "Richy");
		mighty.signForm(*shrubber);
		mighty.signForm(*robotomy);
		mighty.signForm(*pardon);
		mighty.executeForm(*shrubber);
		mighty.executeForm(*pardon);
		expected = "Intern creates Unsigned form Shrubbery Creation: level to sign - 145; level to execute - 137\n"
			"Intern creates Unsigned form Robotomy Request: level to sign - 72; level to execute - 45\n"
			"Intern creates Unsigned form Presidential Pardon: level to sign - 25; level to execute - 5\n"
			"Mighty signed Shrubbery Creation\n"
			"Mighty signed Robotomy Request\n"
			"Mighty signed Presidential Pardon\n"
			"Mighty executed Shrubbery Creation\n"
			"Toriel has been pardoned by Zaphod Beeblebrox\n"
			"Mighty executed Presidential Pardon\n"
			"VVZZZSSS-VVZZZSSS-VVZZZSSS-WWWRRRRM\n";
		cerrExpected = "There is no saweerwdd fsefesf form\n";

		const std::ifstream	file("gallery_shrubbery");
...
TEST_START(Bureaucrat_executeForm)
	...
	TEST_LOGIC_START
		...
		if (strStr.str().rfind("robotomized") == strStr.str().npos)
			...
			expected += "Bobby has been robotomized\n";
		expected += "Handy executed Robotomy Request\n";
		std::cout.rdbuf(ob);
		std::cout << strStr.str();
	TEST_LOGIC_END
	TEST_EMERGENCY_START
